...
from skimage.feature import greycomatrix, greycoprops  # 包含了GLCM的求取方法
...
def calcu_glcm(img, vmin=0, vmax=255, nbit=64, slide_window=5, step=[2], angle=[0]):
    ...
    # 进行灰度压缩，将灰度级数从256转换到64
    bins = np.linspace(mi, ma + 1, nbit + 1)  # 主要用来创建等差数列
    ...
    img2 = cv2.copyMakeBorder(img1, floor(slide_window / 2), floor(slide_window / 2)
                              ...
                              cv2.BORDER_REPLICATE)  # 图像扩充 ，解决拼接像素不存在的问题

    patch = np.zeros((slide_window, slide_window, h, w), dtype=np.uint8)
    patch = image_patch(img2, slide_window, h, w)

    # Calculate GLCM (7, 7, 512, 512) --> (64, 64, 512, 512) 7*7 是滑动窗口的大小，对每个滑动窗口求取他的GLCM，就变成了64*64
    ...
    for i in range(patch.shape[2]):
        for j in range(patch.shape[3]):
            glcm[:, :, :, :, i, j] = skimage.feature.graycomatrix(patch[:, :, i, j], step, angle, levels=nbit)
